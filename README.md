...
```
   ...
   refract  ── parses Ruby with Prism ── persists to per-project SQLite ── serves LSP queries
              │
              └─ also: refract --mcp  (30 tools for AI agents)
```
...
- **AI-native** — `refract --mcp` exposes 30 tools your agent can call directly
...
Start with `refract --mcp`. Exposes 30 tools for AI agent integration.
