...
const TOOLS = [_]ToolEntry{
    .{ .name = "resolve_type", .description = "Resolve the inferred type of a local variable at a source position", .schema = schemas.schema_resolve_type },
    .{ .name = "class_summary", .description = "Get methods, constants, and mixins for a class or module", .schema = schemas.schema_class_summary },
    .{ .name = "method_signature", .description = "Get the full signature and parameter types of a method", .schema = schemas.schema_method_signature },
    .{ .name = "find_implementations", .description = "Find all classes that define a given method name", .schema = schemas.schema_find_implementations },
    .{ .name = "workspace_symbols", .description = "Search symbols across the entire workspace by name", .schema = schemas.schema_workspace_symbols },
    .{ .name = "type_hierarchy", .description = "Get ancestor chain and known descendants of a class", .schema = schemas.schema_type_hierarchy },
    .{ .name = "association_graph", .description = "Get ActiveRecord associations (has_many, belongs_to, etc) for a class", .schema = schemas.schema_association_graph },
    .{ .name = "diagnostics", .description = "Get parse and semantic diagnostics for a file", .schema = schemas.schema_diagnostics },
    .{ .name = "get_symbol_source", .description = "Get the source code of a class method", .schema = schemas.schema_get_symbol_source },
    .{ .name = "grep_source", .description = "Search for literal text across all workspace source files with surrounding context", .schema = schemas.schema_grep_source },
    .{ .name = "i18n_lookup", .description = "Find i18n translation keys and their values", .schema = schemas.schema_i18n_lookup },
    .{ .name = "list_by_kind", .description = "List all symbols of a given kind (class, module, def, etc)", .schema = schemas.schema_list_by_kind },
    .{ .name = "find_unused", .description = "Find symbols with no recorded call sites (static dead-code approximation)", .schema = schemas.schema_find_unused },
    .{ .name = "get_file_overview", .description = "List all symbols in a file ordered by line (flat, fast overview)", .schema = schemas.schema_get_file_overview },
    .{ .name = "list_validations", .description = "List ActiveRecord validation calls for a class", .schema = schemas.schema_list_validations },
    .{ .name = "list_callbacks", .description = "List ActiveRecord/ActionController callbacks for a class", .schema = schemas.schema_list_callbacks },
    .{ .name = "concern_usage", .description = "Find all classes that include/prepend/extend a given module or concern", .schema = schemas.schema_concern_usage },
    .{ .name = "find_references", .description = "Find all recorded call-site references to a method or symbol name", .schema = schemas.schema_find_references },
    .{ .name = "explain_symbol", .description = "Get a full picture of a method: signature, callers, and diagnostics in one call", .schema = schemas.schema_explain_symbol },
    .{ .name = "workspace_health", .description = "Get workspace quality metrics: file counts, type coverage, diagnostic summary, schema version", .schema = schemas.schema_workspace_health },
    .{ .name = "test_summary", .description = "List discovered tests in a file with their kind (rspec/minitest) and line numbers", .schema = schemas.schema_test_summary },
    .{ .name = "list_routes", .description = "List all Rails route helpers with controller and action details", .schema = schemas.schema_list_routes },
    .{ .name = "refactor", .description = "Apply refactoring operations (extract_method, extract_variable) to source code", .schema = schemas.schema_refactor },
    .{ .name = "available_code_actions", .description = "Get list of available code actions at a specific location", .schema = schemas.schema_available_code_actions },
    .{ .name = "explain_type_chain", .description = "Explain how a local variable's type was inferred — shows chain from source (RBS, YARD, literal, chain)", .schema = schemas.schema_explain_type_chain },
    .{ .name = "resolve_constant", .description = "Resolve a constant reference to its declaration using Ruby's lexical+ancestor lookup (nesting-aware) — disambiguates same-named constants across namespaces", .schema = schemas.schema_resolve_constant },
    .{ .name = "overlay_annotate", .description = "Add a gated overlay tweak (note/tag/concept/edge/type-override/diagnostic-suppress) — branch-scoped, reversible, audited. The only writable graph surface; derived facts stay immutable", .schema = schemas.schema_overlay_annotate },
    .{ .name = "overlay_list", .description = "List live overlay tweaks (node/edge/type/suppress) for the current project and branch (plus project-global)", .schema = schemas.schema_overlay_list },
    .{ .name = "overlay_revert", .description = "Soft-delete (revert) an overlay tweak by id", .schema = schemas.schema_overlay_revert },
    .{ .name = "overlay_promote", .description = "Promote branch-scoped overlay tweaks to project-global (visible on every branch and worktree)", .schema = schemas.schema_overlay_promote },
};
