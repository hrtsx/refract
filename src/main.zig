...
const selftest = @import("cli/selftest.zig");
...
pub fn main(init: std.process.Init) !void {
    ...
    if (args.len >= 2 and std.mem.eql(u8, args[1], "--sandbox-exec")) {
        try selftest.runSandboxExec(alloc, io, args);
    ...
    if (flag_self_test) {
        try selftest.runSelfTest(io, alloc);
        return;
    }

    if (bench_sorbet_root != null or bench_steep_root != null) {
        try selftest.runBenchSorbetSteep(io, alloc, bench_sorbet_root, bench_steep_root);
        return;
    }
