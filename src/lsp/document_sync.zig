...
pub fn handleDidOpen(self: *Server, msg: types.RequestMessage) void {
    ...
    if (td.get("text")) |txt_val| {
        if (txt_val == .string) {
            // Register the URI in open_docs BEFORE indexSource creates the
            // file row. cleanupStale (bg thread) snapshots open_docs to build
            // its keep-set, so doing this in the opposite order races: bg can
            // snapshot before the put and then delete the just-created row
            // because the file isn't on disk and the URI isn't yet "kept".
            const doc_key = self.alloc.dupe(u8, uri) catch return;
