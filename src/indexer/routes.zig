...
const routes_roda = @import("routes_roda.zig");
const RouteInfo = routes_common.RouteInfo;
...
pub fn indexRoutesWithPath(db: db_mod.Db, file_id: i64, source: []const u8, backing_alloc: std.mem.Allocator, file_path: []const u8) void {
    ...
    if (!has_routes) {
        routes_roda.indexRoda(db, file_id, &parser, @ptrCast(@alignCast(root)), alloc, file_path);
    }
}
