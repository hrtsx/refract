...
pub fn init(self: Db) DbError!void {
    ...
    // Deferred receiver-scoped return-type resolution: `@ivar = recv.method` assignments
    // whose type needs the COMPLETE symbol table to resolve (method's return on recv's
    // class). Captured at index time, resolved by the post-index flow-typing pass — moving
    // it out of the racy single-pass index-write makes the result deterministic.
    try self.exec(
        \\CREATE TABLE IF NOT EXISTS pending_recv_returns (
        \\  file_id     INTEGER NOT NULL REFERENCES files(id) ON DELETE CASCADE,
        \\  class_id    INTEGER,
